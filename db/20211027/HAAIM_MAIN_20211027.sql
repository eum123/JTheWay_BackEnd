...
TRUNCATE((sum(if(e.goal_score < u.score,0,1)) / count(u.status))*100,0)  pass, 
...
count(member_no)  -- ****************************칼럼명 변경 (members -> member_no)
from class_members
where member_type=0 and status is null
-- group by class_no
-- ) a
...
-- 일별 클래스별멤버 출결 정보 
-- ****************************칼럼명 변경 (members -> member_no)
select a.date, 
round(sum(if(a.attendance=1,1,0))/(select count(member_no) from class_members where member_type=0 and status is null) * 100)  tot_atd, 
round(sum(if(a.attendance != 1, 1, 0)) /(select count(member_no) from class_members where member_type=0 and status is null) * 100) tot_absent
from class_mngt a
group by a.date;
...
SELECT A.CLASS_NO, A.CDATE, A.CTIME, (SELECT concat(grade,'> ', course, '> ', getCurNm(large_category, medium_category)) FROM haaim_db.curriculum WHERE CNO=A.CUR_ID) AS TITLE, A.TYPE_GROUP, A.STATUS
FROM class_curriculum A
WHERE date_format(A.CDate,'%Y-%m-%d') IN (
SELECT
	ADDDATE( '20210404', - DAYOFWEEK('20210404') + 1 ) AS DAY
UNION ALL SELECT 
	ADDDATE( '20210404', - DAYOFWEEK('20210404') + 2 ) AS DAY
...
AND CLASS_NO IN (SELECT CLASS_NO FROM haaim_db.class_members WHERE MEMBER_TYPE=0 AND MEMBER_NO=1)
ORDER BY A.CDATE, A.CTIME,A.CLASS_NO, A.CUR_ID, A.TYPE_GROUP, A.STATUS
;
